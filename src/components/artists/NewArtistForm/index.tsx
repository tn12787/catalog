...
import {
  CreateSingleArtistVars,
  SingleArtistVars,
} from 'queries/artists/types';
...
const NewArtistForm = ({ existingArtist }: Props) => {
  ...
  const { mutateAsync: createArtist, isLoading: createLoading } = useMutation(
    createSingleArtist,
    {
      onSuccess: () => {
  ...
  const { mutateAsync: updateArtist, isLoading: updateLoading } = useMutation(
    updateSingleArtist,
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['artists']);
  ...
  return (
    ...
    >
      <Stack py={8} spacing={3} width="90%" maxW="container.lg">
        <Heading>
          {existingArtist ? 'Edit Artist' : 'Create a new artist'}
        </Heading>
        <Text>
          {existingArtist
        ...
        <Stack
          as="form"
          onSubmit={handleSubmit(existingArtist ? onUpdate : onCreate)}
          width="100%"
        >
          <Card width="100%">
